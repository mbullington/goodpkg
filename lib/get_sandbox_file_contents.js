...
module.exports = function getSandboxFileContents(config) {
  ...
  const yarnrc = path.join(HOME, ".yarnrc");
  ...
  const nodeGypPath = path.join(HOME, ".node-gyp");
  const nodeGypCachePath = path.join(HOME, "Library/Caches/node-gyp");
  ...
  (allow file-read*
      ...
      ;; Various user settings, TZ info, etc.
      (subpath "/etc")
      (subpath "/private/etc")
  ...
  (allow file-read* file-write*
      ...
      ${config.writeYarnrc ? `(literal "${yarnrc}")` : ""}
